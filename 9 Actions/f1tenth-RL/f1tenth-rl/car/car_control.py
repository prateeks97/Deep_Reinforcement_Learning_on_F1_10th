...
#########################################################################################
	...
	# Normal actions increase speed, decrease speed and safety without steering

    def forward_increase(self):
        self.send_drive_command(self.sensors.current_velocity + 0.5, 0)
	
    def backward(self):
        self.send_drive_command(-self.max_speed/self.backward_speed_reduction, 0)

    def forward_decrease(self):
        self.send_drive_command(self.sensors.current_velocity - 0.5, 0)

    def cruise_same_speed(self):
        self.send_drive_command(self.sensors.current_velocity, 0)	

    def stop(self):
        self.send_drive_command(0, 0)

	# Left and right turns with same speed
    def turn_right(self):
        self.send_drive_command(self.sensors.current_velocity, -self.max_steering)

    def turn_left(self):
        self.send_drive_command(self.sensors.current_velocity, self.max_steering)

	# Left and right turns while increasing speed
    def turn_right_increase(self):
        self.send_drive_command(self.sensors.current_velocity + 0.5, -self.max_steering)

    def turn_left_increase(self):
        self.send_drive_command(self.sensors.current_velocity + 0.5, self.max_steering)

	# Left and right turns while decreasing speed
    def turn_right_decrease(self):
        self.send_drive_command(self.sensors.current_velocity - 0.5, -self.max_steering)

    def turn_left_decrease(self):
        self.send_drive_command(self.sensors.current_velocity - 0.5, self.max_steering)
...
#########################################################################################
	...
	# # New added action config
    # def fast_lightly_right(self):
    #     self.send_drive_command((self.max_speed*2)/self.steering_speed_reduction, -self.max_steering/self.lightly_steering_reduction)

    # def fast_lightly_left(self):
    #     self.send_drive_command((self.max_speed*2)/self.steering_speed_reduction, self.max_steering/self.lightly_steering_reduction)

    # def fast_forward(self):
    #     self.send_drive_command((self.max_speed*2)/self.max_speed_reduction, 0)
    ...
    def drive_command_runner(self):
        while True:
            ...
            # print(self.sensors.current_velocity)
